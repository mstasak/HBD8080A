...
TSTNUM LXI  H,0       ;*** TSTNUM ***
       ...
       RST  5         ;A NUMBER
TN1    CPI  0o60      ;IFF NOT, RETURN 0 IN 
       RC             ;B AND HL
       CPI  0o72      ;IFF NUMBERS, CONVERT 
       RNC            ;TO BINARY IN HL AND 
       MVI  A,0o360   ;SET A TO # OF DIGITS
...
TAB1   = *            ;DIRECT COMMANDS 
       .byte "LIST",   >LIST   | 0x80, <LIST
       .byte "RUN",    >RUN    | 0x80, <RUN
       .byte "NEW",    >NEW    | 0x80, <NEW
       .byte "LOAD",   >DLOAD  | 0x80, <DLOAD
       .byte "SAVE",   >DSAVE  | 0x80, <DSAVE
       .byte "BYE",              0x80, 0x00 ;GO BACK TO CPM
TAB2   = *            ;DIRECT/STATEMENT
       .byte "NEXT",   >NEXT   | 0x80, <NEXT
       .byte "LET",    >LET    | 0x80, <LET
       .byte "OUT",    >OUTCMD | 0x80, <OUTCMD
       .byte "POKE",   >POKE   | 0x80, <POKE
       .byte "WAIT",   >WAITCM | 0x80, <WAITCM
       .byte "IF",     >IFF    | 0x80, <IFF
       .byte "GOTO",   >GOTO   | 0x80, <GOTO
       .byte "GOSUB",  >GOSUB  | 0x80, <GOSUB
       .byte "RETURN", >RETURN | 0x80, <RETURN
       .byte "REM",    >REM    | 0x80, <REM
       .byte "FOR",    >FOR    | 0x80, <FOR
       .byte "INPUT",  >INPUT  | 0x80, <INPUT
       .byte "PRINT",  >PRINT  | 0x80, <PRINT
       .byte "STOP",   >STOP   | 0x80, <STOP
	   .byte           >DEFLT  | 0x80, <DEFLT
       .byte "YOU CAN ADD MORE" ;COMMANDS BUT
                      ;REMEMBER TO MOVE DEFAULT DOWN.
TAB4   = *            ;FUNCTIONS 
       .byte "RND",    >RND    | 0x80, <RND
       .byte "INP",    >INP    | 0x80, <INP
       .byte "PEEK",   >PEEK   | 0x80, <PEEK
       .byte "USR",    >USR    | 0x80, <USR
       .byte "ABS",    >ABS    | 0x80, <ABS
       .byte "SIZE",   >SIZE   | 0x80, <SIZE
       .byte           >XP40   | 0x80, <XP40
       .byte "YOU CAN ADD MORE" ;FUNCTIONS BUT REMEMBER
                      ;TO MOVE XP40 DOWN
TAB5   = *            ;"TO" IN "FOR" 
       .byte "TO",     >FR1    | 0x80, <FR1
       .byte           >QWHAT  | 0x80, <QWHAT
TAB6   = *            ;"STEP" IN "FOR" 
       .byte "STEP",   >FR2    | 0x80, <FR2
       .byte           >FR3    | 0x80, <FR3
...
EX1    LDAX D         ;IFF FOUND '.' IN STRING
       ...
       CPI  0o56      ;WE DECLARE A MATCH
       JZ   EX3 
       INX  H         ;HL->TABLE 
       CMP  M         ;IFF MATCH, TEST NEXT 
       JZ   EX1 
       MVI  A,0o177   ;ELSE, SEE IFF BIT 7
...
EX3    MVI  A,0o177   ;PARTIAL MATCH, FIND 
...
GL1    CALL CHKIO     ;CHECK KEYBOARD
       ...
       CPI  0o177     ;DELETE LST CHARACTER?
       JZ   GL3       ;YES 
       CPI  0o12      ;IGNORE LF 
       JZ   GL1 
       ORA  A         ;IGNORE NULL 
...
GL4    CALL CRLF      ;REDO ENTIRE LINE
       MVI  A,0o136   ;CR, LF AND UP-ARROW 
       JMP  GETLN 
;* 
FNDLN  MOV  A,H       ;*** FNDLN *** 
       ORA  A         ;CHECK SIGN OF HL
...
QT3    RST  1         ;IS IT A ' ? 
       .byte   0o47 
       .byte   5
       MVI  A,0o47    ;YES, DO SAME
       JMP  QT1       ;AS IN " 
QT4    RST  1         ;IS IT BACK-ARROW? 
       .byte   0o137
       .byte   0o10 
       MVI  A,0o215    ;YES, 0DHWITHOUT LF!!
...
PN6    MOV  A,E       ;CHECK DIGIT IN E
       CPI  0o12      ;10 IS FLAG FOR NO MORE
       POP  D 
       RZ             ;IFF SO, RETURN 
...
PRTLN  LDAX D         ;*** PRTLN *** 
       ...
       CALL PRTNUM
       MVI  A,0o40    ;FOLLOWED BY A BLANK 
       RST  2 
...
OC3    POP  PSW       ;GET OLD A BACK
	   ...
	   OUT  2         ;for emulator, port is always ready
       CPI  '\r'      ;WAS IT A 'CR'?
       ;JNZ  DONE      ;NO, DONE
       JNZ  IDONE     ;NO, DONE
       ;MVI  E,'\n'    ;GET LINEFEED
...
CHKIO  ;PUSH B         ;SAVE B ON STACK
       ;PUSH D         ;AND D
       ;PUSH H         ;THEN H
CHKI1  IN 3           ;get kb status
	   ANI 2
	   JZ IDONE       ;nothing waiting, return Z=1
	   IN 2           ;get character
...
;       JMP  CHKIO     ;AND GET ANOTHER CHARACTER
       JMP  CHKI1     ;AND GET ANOTHER CHARACTER
...
OCSW   .byte   0xFF      ;SWITCH FOR OUTPUT
...
MSG1   .byte   0x7F,0x7F,0x7F,"SHERRY BROTHERS TINY BASIC VER. 3.1\r" 
INIT   MVI  A, 0xFF
       STA  OCSW      ;TURN ON OUTPUT SWITCH 
       MVI  A,0x0C     ;GET FORM FEED 
       RST  2         ;SEND TO CRT 
...
RSTBL:
       ...
       JMP  TC1       ;REST OF THIS IS AT TC1
;* 
CRLF:  =	0x0E     ;EXECUTE TIME LOCATION OF THIS INSTRUCTION.
	   MVI  A, 0x0D   ;*** CRLF ***
;* 
       PUSH PSW       ;*** OUTC OR RST 2 *** 
       LDA  OCSW      ;PRINT CHARACTER ONLY
...
;* 
       ...
       .byte   "AN"
;* 
SS1:   =    0x0028       ;EXECUTE TIME LOCATION OF THIS INSTRUCTION.
	   LDAX D         ;*** IGNBLK/RST 5 ***
       CPI  0o40       ;IGNORE BLANKS 
       RNZ            ;IN TEXT (WHERE DE->)
       INX  D         ;AND RETURN THE FIRST
       JMP  SS1       ;NON-BLANK CHAR. IN A
;* 
...
RST1:  =    8	;LOCATION FIRST REATART ROUTINE

EOT:   =    0x0040	;LAST LOC TO BE FILLED
...
NXT:    LDAX	D
	...
	JMP	START
       ;.ORG  0x0F00
TXTEND =  0x0F00 ;*        ;TEXT SAVE AREA ENDS 
VARBGN = TXTEND ;.storage   2*27      ;VARIABLE @(0)
       .storage   1         ;EXTRA BYTE FOR BUFFER
BUFFER = VARBGN + 2 * 27 + 1 ;.storage   80        ;INPUT BUFFER
BUFEND =  BUFFER + 80 ;*         ;BUFFER ENDS
       .storage   40        ;EXTRA BYTES FOR STACK
STKLMT = BUFEND + 40;  *         ;TOP LIMIT FOR STACK
       ;.ORG  0x2000
STACK  = 0x2000  ;*         ;STACK STARTS HERE
       ;.END
